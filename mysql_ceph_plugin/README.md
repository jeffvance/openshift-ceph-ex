...
We're using a simple [pod spec](mysql-ceph-pod.yaml) which uses the same mysql image, defines a container volume mount that references the ceph-rbd persistent storage plugin, and specifies the ceph monitor's ip address (which is the ip address of the ceph AIO container's host VM).
...
An alternative to running the mysql container privileged is to set selinux to permissive mode (*setenforce 0*) on *all* schedulable ose-nodes; however, running the mysql container privileged is likely preferred.
