...
The mysql container ID is 9a43017dbebf. More details on this container are availble via *docker inspect <container-ID-or-name>*. Log information is shown via *docker logs <container-ID>*, and via *systemctl status openshift-node -l* and *journalctl -xe -u openshift-node docker*.
